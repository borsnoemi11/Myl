...
public class LexerTests
{
    ...
    {
        foreach (var t in GetTokens().Concat(GetSeparatorTokens()))
        {
    ...
    private static IEnumerable<(SyntaxKind kind, string text)> GetTokens()
    {
        var fixedTokens = Enum.GetValues(typeof(SyntaxKind))
                              ...
                              .Where(t => t.text != null)
                              .Select(t => (kind: t.kind, text: t.text!));
    ...
    private static bool RequiresSeparator(SyntaxKind t1Kind, SyntaxKind t2Kind)
    {
        ...
        var t1IsIdentifier = t1Kind == SyntaxKind.IdentifierToken;
        var t2IsIdentifier = t2Kind == SyntaxKind.IdentifierToken;

        if (t1IsIdentifier && t2IsIdentifier)
            return true;

        if (t1IsKeyword && t2IsKeyword)
            return true;

        if (t1IsKeyword && t2IsIdentifier)
            return true;

        if (t1IsIdentifier && t2IsKeyword)
            return true;
